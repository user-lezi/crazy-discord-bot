...
import { colorDistanceSq } from "../../../util/color";
import { getCachedGuildMembers } from "../../../functions/getCachedGuildMembers";
import getImageAverageColor from "../../../functions/getImageAverageColor";
import mapWithConcurrency from "../../../functions/mapWithConcurrency";
...
export async function guildIconImageCommand(
  ...
  const avatarEntries = await mapWithConcurrency(
    ...
    async (member): Promise<AvatarEntry> => {
      ...
      return { member, image, color: getImageAverageColor(image) };
    },
  );
